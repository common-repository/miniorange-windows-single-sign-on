<?php
/** miniOrange Windows SSO enables user to perform Single Sign On with any SAML 2.0 enabled Identity Provider.
 ...
 * @package 		miniOrange Windows SSO
...
class Customersaml {
	...
	function create_customer() {
		...
		$fields = array (
				'areaOfInterest' => 'WP miniOrange Windows SSO Plugin',
				'email' => $this->email,
				'password' => $password
		);
	...
	function send_otp_token($email, $phone, $sendToEmail = TRUE, $sendToPhone = FALSE) {
		...
		if ($sendToEmail) {
			$fields = array (
					...
					'email' => $email,
					'authType' => 'EMAIL',
					'transactionName' => 'WP miniOrange Windows SSO  SSO Plugin'
			);
		} else {
			$fields = array (
					'customerKey' => $customerKey,
	...
	function submit_contact_us($email, $phone, $query) {
		...
		$current_user = wp_get_current_user();
		$query = '[WP miniOrange Windows SSO Plugin] ' . $query;
		$fields = array (
	...
	function send_email_alert($email,$phone,$message){
		...
		$subject            = "Feedback: Single Sign On for Windows";
		$site_url=site_url();
		...
		$query        = '[ Single Sign On for Windows ]: ' . $message;
