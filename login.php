<?php
/*
Plugin Name: miniOrange Windows SSO
Plugin URI: http://miniorange.com/
Description: miniOrange Windows SSO enables user to perform Single Sign On with any SAML 2.0 enabled Identity Provider.
Version: 5.0.3
Author: miniOrange
...
class saml_mo_login {
	...
	function miniorange_sso_menu() {
		...
		add_menu_page( 'MO SAML Settings ' . __( 'Configure SAML Identity Provider for SSO', 'mo_saml_settings' ), 'miniOrange Windows SSO', 'administrator', $slug, array(
			$this,
			'mo_login_widget_saml_options'
		), plugin_dir_url( __FILE__ ) . 'images/miniorange.png' );
        add_submenu_page( $slug	,'miniOrange Windows SSO'	,'Plugin Configuration','manage_options','mo_saml_settings'
            , array( $this, 'mo_login_widget_saml_options'));
        add_submenu_page( $slug	,'miniOrange Windows SSO'	,'Licensing Plans','manage_options','mo_saml_settings&amp;tab=licensing'
	...
	function mo_saml_redirect_for_authentication( $relay_state ) {
			...
			if ( mo_saml_is_sp_configured() && ! is_user_logged_in() ) {
				...
				$acsUrl         = site_url() . "/";
				$issuer         = site_url() . '/wp-content/plugins/miniorange-windows-single-sign-on/';
